...
<HighlightBlock type="warning">
  **Multiple Toasters**: If you use multiple toasters, each toaster mustbe provided with a
  unique `toasterId` and toast functions must be called with a `toasterId` prop to target the correct toaster (exceptions apply to specific functions).
</HighlightBlock>
