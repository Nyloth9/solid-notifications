...
Accepts a [toast config](/props#toast-config) props as well as `id` and `toasterId` ([other config](/props#other-config)). Also accepts `content` props when updating a toast.
