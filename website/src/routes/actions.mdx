...
The dismiss function accepts an optional options object with the following properties:
    ...
    - **reason**: A string or custom value that can be used to track why a toast was dismissed (e.g., "user_action"). This value will be passed to the toast's `onExit` callback.

If no arguments are provided, the function will dismiss all toasts across all toasters.

Dismiss all toasts across all toasters:

javascript
Copy
dismiss();
Dismiss all rendered toasts across all toasters, keeping queued toasts:

javascript
Copy
dismiss({ keepQueued: true });
Dismiss a specific toast in a specific toaster:

javascript
Copy
dismiss({ id: "toastId", toasterId: "toasterId" });
Dismiss all toasts in a specific toaster:

javascript
Copy
dismiss({ toasterId: "toasterId" });
Dismiss all rendered toasts in a specific toaster, keeping queued toasts:

javascript
Copy
dismiss({ toasterId: "toasterId", keepQueued: true });
