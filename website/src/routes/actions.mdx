...
The `update` function allows you to modify an existing toast's content or appearance. It takes one argument; an object containing the toast's ID and the new content or options.

<HighlightBlock type="info">
  The new body, i.e. `content` of the toast, should be passed as a property in
  the `options` object.
</HighlightBlock>
