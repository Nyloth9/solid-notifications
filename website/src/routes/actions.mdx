...
The `getQueue` function can be called with an optional `toasterId` argument to target a specific toaster. The `toasterId` is required if there are multiple toasters.

<Code>

```tsx
const queue = getQueue(); // if there is only one toaster
// or
